...
def render_report(report: dict, output: Path, avatars_dir: Path) -> Path:
    ...
    for rank, pick in enumerate(report.get("picks", []), 1):
        ...
        evidence = "".join(f'<li><b>{item.get("grade", "—")}</b> {item.get("title", "—")} <span>{item.get("source", "")}</span></li>' for item in pick.get("evidence", []))
